...
function parseStream (instream, extension) {
  ...
  rl.on('line', function (line) {
    ...
    if (line.indexOf('#') === 0) {
      ...
      // ##contig=<ID=Sample1,length=1234>
      // ##customHeader=someData
      if (line.match(/^##(.*)=<(.*)>/)) {
        var lineData = /^##(.*)=<(.*)>/.exec(line)

        var result = {}
        lineData[2].split(',').forEach((kvPair) => {
          var kvArray = kvPair.split('=')
          result[kvArray[0]] = kvArray[1]
        })

        if (headerDataMap.has(lineData[1])) {
          var existingData = headerDataMap.get(lineData[1])
          existingData.push(result)
          headerDataMap.set(lineData[1], existingData)
        } else {
          headerDataMap.set(lineData[1], [result])
        }
      } else if (line.match(/^##(.*)=(.*)/)) {
        lineData = /^##(.*)=(.*)/.exec(line)
        headerDataMap.set(lineData[1], lineData[2])
      }
      ...
      var varData = {
        ...
        __HEADER__: mapToObj(headerDataMap)
      }
